...
- The user's early intervention ("Is `npm root -g` safe? What about Homebrew/pnpm/bun?") redirected the design toward `import.meta.url` self-discovery before any code was written — avoiding a fragile subprocess-based approach that would have required package-manager detection logic.
...
- No significant friction from the user side. The early design question about `npm root -g` safety was well-timed and prevented wasted work.
